...
class Seg(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            num_selected_modalities = num_modalities

        # 随机选择具体的通道索引
        selected_indices = random.sample(range(num_modalities), num_selected_modalities)
        unselected_indices = [idx for idx in range(num_modalities) if idx not in selected_indices]

        # 选择这些通道并沿着第一个维度拼接
        # [num_selected_modalities, B, C, H, W]
        ...
        # print(_x[0][0].shape) # torch.Size([32, 128, 128])
        # print(_x_all[0][0].shape) # torch.Size([1, 32, 128, 128])
